...
# Multi value arguments:
#   DEPS            - List of linkable dependencies for the library
#   SOURCE_DIRS     - List of source file directories to pull files
#                     files from. These use a priority system, with
#                     files only being added once.
#   HEADER_DIRS     - List of header file directories to pull
#                     headers from. These use a priority system, with
#                     files only being added once. The currently relative
#                     include is added by default.
#   SOURCE_FILTER   - Source files to ignore
#
# Implicit arguments from parent scope:
#   TARGET_LANGUAGE - module target language (c or c++)
#
function(coda_add_module_flex MODULE_NAME)
    cmake_parse_arguments(
        ARG                                             # prefix
        ""                                              # options
        "VERSION"                                       # single args
        "DEPS;SOURCE_DIRS;HEADER_DIRS;SOURCE_FILTER"    # multi args
        "${ARGN}"
    )
    if (ARG_UNPARSED_ARGUMENTS)
        message(FATAL_ERROR "received unexpected argument(s): ${ARG_UNPARSED_ARGUMENTS}")
    ...
    # Default to local globbing - potentially used alongside flexible headers
    set(local_sources "")
    set(seen_files "")
    if (ARG_SOURCE_DIRS)
        # Add source files in order of direct priority
        foreach(source_dir ${ARG_SOURCE_DIRS})
            # Parse for absolute directories. Directories must be absolute (/path/to/file/) or relative to the current dir
            if (IS_ABSOLUTE ${source_dir})
                set(abs_source ${source_dir})
            else()
                set(abs_source "${CMAKE_CURRENT_SOURCE_DIR}/${source_dir}")
            endif()

            if (NOT EXISTS "${abs_source}")  # Fixed: Add quotes around ${abs_source}
                message(WARNING "Provided source directory does not exist, attempting module build with remaining sources: ${source_dir}")
                continue()
            endif()

            # Find all the source files, relative to the source directory provided
            file(GLOB_RECURSE sources 
                "${abs_source}/*.cpp"
                "${abs_source}/*.c")

            # CUDA setup - Largely untested
            if(GPU_ENABLED)
                set(expected_cuda_kernel "${abs_source}/kernels")
                if (EXISTS "${expected_cuda_kernel}")
                    file(GLOB_RECURSE abs_cuda
                        "${expected_cuda_kernel}/*.cu")
                    list(APPEND sources "${abs_cuda}")
                endif()
            endif()

            # Do not add files already in local sources - this may be a cause of instability and require further refinement
            foreach(abs_file ${sources})
                # Crosscheck the file with whats already added to sources 
                get_filename_component(filename "${abs_file}" NAME)
                list(FIND seen_files "${filename}" idx)  # Fixed: Use "${filename}" instead of {filename idx)
                if (idx EQUAL -1)
                    # Assume its the first time we have seen it
                    list(APPEND seen_files "${filename}")

                    # Convert new files to relative path and add to sources
                    file(RELATIVE_PATH rel_file "${CMAKE_CURRENT_SOURCE_DIR}" "${abs_file}")
                    if (EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/${rel_file}")
                        list(APPEND local_sources "${rel_file}")
                    endif()
                endif()
            endforeach()
        endforeach()
    else()
        # If no source directories are provided, we assume only headers are flex. Hence we revert to default behavior.
        file(GLOB_RECURSE local_sources RELATIVE "${CMAKE_CURRENT_SOURCE_DIR}" 
            "source/*.cpp" "source/*.c")

        if (GPU_ENABLED AND EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/kernels")
            file(GLOB_RECURSE cuda_sources RELATIVE "${CMAKE_CURRENT_SOURCE_DIR}" 
                "kernels/*.cu")
            list(APPEND local_sources ${cuda_sources})
        endif()
    endif()
    ...
    # Filter out ignored files
    if (ARG_SOURCE_FILTER)
        filter_files(local_sources "${local_sources}" "${ARG_SOURCE_FILTER}")
    endif()

    set(cuda_sources "")
    if (GPU_ENABLED)
        set(cuda_temp ${local_sources})
        list(FILTER cuda_temp INCLUDE REGEX "\\.cu$")
        set(cuda_sources ${cuda_temp})
    endif()
    ...
    # Add custom headers first as target_include_directories has built in prio
    if (ARG_HEADER_DIRS)
        foreach(header_dir ${ARG_HEADER_DIRS})
            if (IS_ABSOLUTE ${header_dir})
                set(abs_header "${header_dir}")
            else()
                set(abs_header "${CMAKE_CURRENT_SOURCE_DIR}/${header_dir}")
            endif()

            if (EXISTS "${abs_header}") 
                target_include_directories(${target_name} ${lib_type} 
                    "$<BUILD_INTERFACE:${abs_header}>"
                    "$<INSTALL_INTERFACE:${CODA_STD_PROJECT_INCLUDE_DIR}>")
            endif()
        endforeach()
    endif()
    ...
    # Install custom header directories
    if (ARG_HEADER_DIRS)
        foreach(header_dir ${ARG_HEADER_DIRS})
            if (IS_ABSOLUTE ${header_dir})
                set(abs_header_dir "${header_dir}")
            else()
                set(abs_header_dir "${CMAKE_CURRENT_SOURCE_DIR}/${header_dir}")
            endif()

            if (EXISTS "${abs_header_dir}")
                # Get the dirname for install location
                get_filename_component(dir_name "${header_dir}" NAME)
                install(DIRECTORY "${abs_header_dir}/"
                        DESTINATION "${CODA_STD_PROJECT_INCLUDE_DIR}/${MODULE_NAME}/${dir_name}/"
                        ${CODA_INSTALL_OPTION}
                        FILES_MATCHING
                            PATTERN "*.in" EXCLUDE
                            PATTERN "*")
