...
inline static void test_assert_eq(const std::string &testName, const std::u8string &actual,
...
TEST_CASE(testConvert)
{
    ...
    TEST_ASSERT_EQ(str::toType<unsigned long long>("-10", 10), gsl::narrow_cast<unsigned long long>(-10));
...
template <typename T> static constexpr std::u8string::value_type cast8(T ch)
{
...
TEST_CASE(test_string_to_u8string_windows_1252)
{
    // Windows-1252 only characters must be mapped to UTF-8
    {
        ...
        const auto actual = str::to_u8string<str::W1252string>(input);
        const std::u8string expected8{cast8('|'), cast8('\xE2'), cast8('\x82'), cast8('\xAC'),
                                      cast8('|')}; // UTF-8,  "|€|"
        TEST_ASSERT_EQ(actual, expected8);
    ...
    }
    {
        ...
        for (const auto &ch : undefined)
        {
            ...
            // const std::u8string expected8{cast8('|'), cast8('\xEF'), cast8('\xBF'), cast8('\xBD'), cast8('|')};  //
    ...
    }
    {
        ...
        const std::vector<uint8_t> windows1252_characters{0x80,  // EURO SIGN
...
static void test_change_case_(const std::string &testName, const std::basic_string<TChar> &lower,
                              const std::basic_string<TChar> &upper)
{
...
static const wchar_t *classificationText_wide_()
{
    return L"A\x00c9IOU";
} // "wide characters" "AÉIOU"
static std::u16string classificationText_u16()
{
    return u"A\u00c9IOU";
} // UTF-16 "AÉIOU"
static std::u32string classificationText_u32()
{
    return U"A\u00c9IOU";
} // UTF-32 "AÉIOU"

static std::string classificationText_platform()
{
    return sys::Platform == sys::PlatformType::Linux ? toString(classificationText_u8())
                                                     : str::testing::to_string(classificationText_w1252());
}
...
static void test_Encodeding_(const std::string &testName, const coda_oss::u8string &classificationText_u8,
                             const std::string &utf_8, const std::string &iso8859_1, const coda_oss::u8string &utf_8_u8,
                             const coda_oss::u8string &iso8859_1_u8, const std::string &utf_8_view,
                             const std::string &iso8859_1_view)
{
...
TEST_CASE(test_Encoding)
{
    ...
    test_Encodeding_(testName, classificationText_u8(), utf_8, iso8859_1, utf_8_u8, iso8859_1_u8, utf_8_view,
                     iso8859_1_view);
    test_Encodeding_(testName, classificationText_u8(), iso8859_1, utf_8, iso8859_1_u8, utf_8_u8, iso8859_1_view,
                     utf_8_view);
}

TEST_MAIN(TEST_CHECK(testConvert); TEST_CHECK(testBadConvert); TEST_CHECK(testEightBitIntToString);
          TEST_CHECK(testCharToString); TEST_CHECK(test_string_to_u8string_ascii);
          TEST_CHECK(test_string_to_u8string_windows_1252); TEST_CHECK(test_string_to_u8string_iso8859_1);
          TEST_CHECK(test_change_case); TEST_CHECK(test_u8string_to_string); TEST_CHECK(test_u8string_to_u16string);
          TEST_CHECK(test_u8string_to_u32string); TEST_CHECK(test_ASCII); TEST_CHECK(test_Windows1252_WIN32);
          TEST_CHECK(test_Windows1252); TEST_CHECK(test_Encoding);)
