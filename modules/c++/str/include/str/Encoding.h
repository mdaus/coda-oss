...
namespace details
{
template <typename TReturn, typename TChar> inline auto cast(const TChar *s)
{
...
template <typename TBasicStringT, typename TChar> inline auto c_str(const std::basic_string<TChar> &s)
{
    using return_t = typename TBasicStringT::const_pointer;
    return details::cast<return_t>(s.c_str());
}
template <typename TBasicStringT, typename TChar> inline auto str(const std::basic_string<TChar> &s)
{
    return TBasicStringT(c_str<TBasicStringT>(s), s.length()); // avoid extra strlen() call
}
template <typename TBasicStringT, typename TChar> inline TBasicStringT make_string(TChar *p)
{
...
template <typename TBasicString> inline auto to_u8string(const std::string &s) // UTF-8 or Windows-1252
{
    return to_u8string(str::c_str<TBasicString>(s), s.length());
}
template <typename TBasicString> inline auto to_u8string(const std::wstring &s) // UTF-16 or UTF-32
{
    return to_u8string(str::c_str<TBasicString>(s), s.length());
}
...
inline std::string to_native(const coda_oss::u8string &s) // cf., std::filesystem::native(),
