...
#pragma warning(disable                                                                                                \
                : 26460) // The reference argument 's' for function 'str::data<char>' can be marked as const (con.3).
#endif
 ...
 */
CODA_OSS_API size_t replace(std::string &str, const std::string &search, const std::string &replace, size_t start = 0);
 ...
 *  @param  replace String to replace with
 */
CODA_OSS_API void replaceAll(std::string &string, const std::string &search, const std::string &replace);
 ...
 */
CODA_OSS_API std::vector<std::string> split(const std::string &s, const std::string &splitter = " ",
...
template <typename T> inline std::string join(const std::vector<T> &toks, const std::string &with)
{
    if (toks.empty())
        return "";
