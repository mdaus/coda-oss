...
inline char to_w1252_upper_(char ch)
{
    ...
    constexpr uint8_t a_with_grave = 0xe0 /* � */;
    constexpr uint8_t o_with_diaeresis = 0xf6 /* � */;
    if ((u8 >= a_with_grave) && (u8 <= o_with_diaeresis))
    {
        return ch ^ 0x20;
    }
    // U+00F7 � DIVISION SIGN
    constexpr uint8_t o_with_slash = 0xf8 /* � */;
    constexpr uint8_t small_thorn = 0xfe /* � */;
    if ((u8 >= o_with_slash) && (u8 <= small_thorn))
    {
        return ch ^ 0x20;
    }

    constexpr uint8_t y_with_diaeresis = 0xff /* � */;
...
inline char to_w1252_lower_(char ch)
{
    ...
    constexpr uint8_t A_with_grave = 0xc0 /* � */;
    constexpr uint8_t O_with_diaeresis = 0xd6 /* � */;
    if ((u8 >= A_with_grave) && (u8 <= O_with_diaeresis))
    {
        return ch | 0x20;
    }
    // U+00D7 � MULTIPLICATION SIGN 
    constexpr uint8_t O_with_slash = 0xd8 /* � */;
    constexpr uint8_t capital_thorn = 0xde /* � */;
...
{
    auto w1252 = to_w1252string(str::str<std::u8string>(strUtf8));
...
inline void utf8_convert(std::u8string& s, void (*convert)(str::W1252string&))
{
    auto w1252 = to_w1252string(s);
    s = utf8_convert(w1252, convert);
}
void lower(std::u8string& s)
{
    utf8_convert(s, lower);
}
void upper(std::u8string& s)
{
