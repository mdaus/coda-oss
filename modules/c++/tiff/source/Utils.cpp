...
tiff::IFD *tiff::Utils::createGeoTiffIFD(tiff::IFD *ifd)
{
    ...
    tiff::IFDEntry *const doubleParams = ifd->exists("GeoDoubleParamsTag") ? (*ifd)["GeoDoubleParamsTag"] : nullptr;
    tiff::IFDEntry *const asciiParams = ifd->exists("GeoAsciiParamsTag") ? (*ifd)["GeoAsciiParamsTag"] : nullptr;
    ...
    for (unsigned short i = 0; i < numKeys; ++i)
    {
        ...
        {
            throw except::Exception(Ctxt("'GeoKeyDirectoryTag' specified " + std::to_string(numKeys) +
                                         " keys but the IFD entry only had " + std::to_string(geoVals.size()) +
                                         " values"));
        }
        ...
        else if (tiffTagLoc == 34736 && doubleParams)
        {
            ...
            {
                entry->addValue(new tiff::GenericType<double>((*doubleParams)[valueOffset + j]->toString()));
            }
        }
        else if (tiffTagLoc == 34737 && asciiParams)
        {
            ...
            for (unsigned short j = 0; j < count; ++j)
            {
                entry->addValue(new tiff::GenericType<std::string>((*asciiParams)[valueOffset + j]->toString()));
            }
        }
