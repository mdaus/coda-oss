...
struct FooPtrTest
{
    FooPtrTest(mem::SharedPtr<Foo> ptr) : mFooPtr(ptr)
    {
    }
...
{
    BarPtrTest(mem::SharedPtr<Bar> ptr) : FooPtrTest(ptr), mBarPtr(ptr)
    {
    }
...
TEST_MAIN(TEST_CHECK(testNullCopying); TEST_CHECK(testAutoPtrConstructor); TEST_CHECK(testAutoPtrReset);
          TEST_CHECK(testCopying); TEST_CHECK(testAssigning); TEST_CHECK(testSyntax); TEST_CHECK(testCasting);
