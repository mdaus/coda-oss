...
TEST_CASE("TestGetNumSharedElements")
{
    ...
    CHECK(range.getNumSharedElements(0, 100) == static_cast<size_t>(0));

    // Starts at upper bound of 'range' - shouldn't share any elements
    CHECK(range.getNumSharedElements(150, 50) == static_cast<size_t>(0));

    // Intersects 'range' at its lower bound, but doesn't cover the
    // full extent - shares [100, 125]
    CHECK(range.getNumSharedElements(50, 75) == static_cast<size_t>(25));

    // Intersects 'range' at its upper bound but doesn't cover the
    // full extent - shares [120, 150)
    CHECK(range.getNumSharedElements(120, 45) == static_cast<size_t>(30));

    // Lays between the upper and lower bound of 'range' - 'range' should
    // share all elements [120, 135]
    CHECK(range.getNumSharedElements(120, 15) == static_cast<size_t>(15));

    // Covers the entirety of 'range', should share [100, 150)
    CHECK(range.getNumSharedElements(0, 200) == static_cast<size_t>(50));

    // Ranges are the same - should share [100, 150)
    CHECK(range.getNumSharedElements(100, 50) == static_cast<size_t>(50));
}
...
TEST_CASE("TestTouches")
{
    // Ranges do not overlap or touch -- touches(...) returns false
    {
        ...
        CHECK_FALSE(A.touches(B));
        CHECK_FALSE(B.touches(A));
    }
...
TEST_CASE("TestSplit")
{
    // Test splitting all elements
    {
        ...
        CHECK(splitRange.mNumElements == static_cast<size_t>(10));
        CHECK(splitRange.mStartElement == static_cast<size_t>(5));
    }
    ...
    // Test splitting a portion of elements
    {
        ...
        CHECK(splitRange.mNumElements == static_cast<size_t>(5));
        CHECK(splitRange.mStartElement == static_cast<size_t>(10));
    }
    ...
    // Test splitting zero elements
    {
        ...
        CHECK(splitRange.mNumElements == static_cast<size_t>(0));
        CHECK(splitRange.mStartElement == std::numeric_limits<size_t>::max());
    }
