...
{
    TEST_ASSERT(
        math::linear::equals(-std::numeric_limits<double>::infinity(), -std::numeric_limits<double>::infinity()));
    TEST_ASSERT(math::linear::equals(-std::numeric_limits<float>::infinity(), -std::numeric_limits<float>::infinity()));
}
...
TEST_MAIN(TEST_CHECK(positiveInfinityEq); TEST_CHECK(negativeInfinityEq); TEST_CHECK(differentSignsNe);)
