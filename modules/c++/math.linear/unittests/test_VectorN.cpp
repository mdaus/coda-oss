 ...
 *      template<typename _T> math::linear::VectorN<,double><_T> operator*(const math::linear::Matrix2D<_T>& m, const
 * math::linear::VectorN<,double><_T>& v) template<typename _T> math::linear::VectorN<,double><_T> operator*(_T scalar,
 * const math::linear::VectorN<,double><_T>& v) template<typename _T> std::ostream& operator<<(std::ostream& os, const
...
TEST_MAIN(TEST_CHECK(testDefaultConstructor); TEST_CHECK(testScalarConstructor); TEST_CHECK(testRawConstructor);
          TEST_CHECK(testCopyConstructor); TEST_CHECK(testStdVectorConstructor); TEST_CHECK(testAssignmentOperator);
          TEST_CHECK(testScalarAssignment); TEST_CHECK(testStdVectorAssignment); TEST_CHECK(testDotProduct);
          TEST_CHECK(testNorm); TEST_CHECK(testNormalize); TEST_CHECK(testUnit); TEST_CHECK(testOperatorPlusEquals);
          TEST_CHECK(testOperatorPlus); TEST_CHECK(testOperatorMinus); TEST_CHECK(testNegate); TEST_CHECK(testAdd);
          TEST_CHECK(testSubtract); TEST_CHECK(testOperatorMinus); TEST_CHECK(testOperatorTimesEquals);
          TEST_CHECK(testOperatorTimesEqualsScalar); TEST_CHECK(testOperatorTimesScalar);
          TEST_CHECK(testElementDivision); TEST_CHECK(testOperatorTimes); TEST_CHECK(testOperatorDivide);)
