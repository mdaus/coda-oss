...
// character type used by the native encoding of the filesystem: char on POSIX, wchar_t on Windows
    ...
    template <typename TSource> path(const TSource &source)
    {
        *this = source;
    }
    ~path() = default;
    ...
    template <typename TSource> path &operator=(const TSource &source)
    {
        p_ = to_native(source);
        return *this;
    }
    ...
    CODA_OSS_API friend bool operator==(
        const path &lhs, const path &rhs) noexcept // https://en.cppreference.com/w/cpp/filesystem/path/operator_cmp
    {
    ...
    CODA_OSS_API friend std::ostream &operator<<(
        std::ostream &os, const path &p) // https://en.cppreference.com/w/cpp/filesystem/path/operator_ltltgtgt
    {
...
CODA_OSS_API path operator/(const path &lhs,
