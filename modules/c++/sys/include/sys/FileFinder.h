...
struct LogicalPredicate : public FilePredicate
{
    ...
    sys::LogicalPredicate &addPredicate(FilePredicate *filter, bool ownIt = false);
...
struct CODA_OSS_API FileFinder final
{
     ...
     */
    static std::vector<std::string> search(const FilePredicate &filter, const std::vector<std::string> &searchPaths,
...
CODA_OSS_API coda_oss::filesystem::path findFirstFile(const coda_oss::filesystem::path &startingDirectory,
                                                      const coda_oss::filesystem::path &filename);
coda_oss::filesystem::path findFirstDirectory(const coda_oss::filesystem::path &startingDirectory,
...
CODA_OSS_API coda_oss::filesystem::path findRootDirectory(
    const coda_oss::filesystem::path &p, const std::string &rootName,
...
coda_oss::filesystem::path findModuleFile(const coda_oss::filesystem::path &root, const std::string &externalsName,
...
CODA_OSS_API coda_oss::filesystem::path findGITModuleFile( // use current_directory() to find_dotGITDirectory()
    const std::string &externalsName, const coda_oss::filesystem::path &modulePath,
    const coda_oss::filesystem::path &moduleFile);
} // namespace test
} // namespace sys
#endif
