...
struct CODA_OSS_API File
{
     ...
     */
    File(const Path &path, int accessFlags = READ_ONLY, int creationFlags = EXISTING)
    {
        create(path.getPath(), accessFlags, creationFlags);
    }
    File(std::nothrow_t, const coda_oss::filesystem::path &path, int accessFlags = READ_ONLY,
         int creationFlags = EXISTING) noexcept // caller MUST check isOpen()
    {
        create(std::nothrow, path, accessFlags, creationFlags);
    }

    File(const Path &parent, std::string name, int accessFlags = READ_ONLY, int creationFlags = EXISTING)
    {
     ...
     *  \param creationFlags File creation flags
     */
    explicit File(const std::string &str, int accessFlags = READ_ONLY, int creationFlags = EXISTING)
    {
    ...
    void create(std::nothrow_t, const coda_oss::filesystem::path &path, int accessFlags,
...
CODA_OSS_API File make_File(const coda_oss::filesystem::path &, int accessFlags = File::READ_ONLY,
...
CODA_OSS_API std::ifstream make_ifstream(
    ...
    std::ios_base::openmode mode =
        std::ios_base::in); // https://en.cppreference.com/w/cpp/io/basic_ifstream/basic_ifstream
CODA_OSS_API void open(
    std::ifstream &, const coda_oss::filesystem::path &,
    std::ios_base::openmode mode = std::ios_base::in); // https://en.cppreference.com/w/cpp/io/basic_ifstream/open
