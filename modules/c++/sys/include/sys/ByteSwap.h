...
template <typename TByte> inline void byteSwap_(TByte *buffer, size_t elemSize, size_t numElems)
{
...
namespace details
{
template <typename T> inline void check_elemSize(size_t elemSize)
{
...
template <typename T> inline auto make_span(coda_oss::span<const std::complex<T>> s)
{
...
template <typename T> inline auto make_span(coda_oss::span<std::complex<T>> s)
{
    // static_assert(std::is_floating_point<T>::value, "std::complex<T> should use floating-point");
...
template <typename T> inline void byteSwap(T *buffer, size_t elemSize, size_t numElems)
{
...
template <typename T>
inline void byteSwap(std::complex<T> *buffer, size_t elemSize,
...
template <typename T> inline auto byteSwap(coda_oss::span<T> buffer)
{
...
template <typename T> inline auto byteSwap(coda_oss::span<std::complex<T>> buffer)
{
 ...
 */
coda_oss::span<const coda_oss::byte> CODA_OSS_API byteSwap(coda_oss::span<const coda_oss::byte> buffer, size_t elemSize,
...
template <typename U> inline void byteSwap(const byte *buffer, size_t elemSize, size_t numElems, U *outputBuffer)
{
    byteSwap_(buffer, elemSize, numElems, outputBuffer);
}
template <typename U> inline void byteSwap(const ubyte *buffer, size_t elemSize, size_t numElems, U *outputBuffer)
{
    byteSwap_(buffer, elemSize, numElems, outputBuffer);
}
...
template <typename T, typename U>
inline void byteSwap(const std::complex<T> *buffer, size_t elemSize, size_t numElems,
...
template <typename T> inline auto byteSwap(coda_oss::span<const T> buffer, coda_oss::span<coda_oss::byte> outputBuffer)
{
    static_assert(details::is_byte_swappable<T>(), "T should not be a 'struct'");
...
template <typename T> inline auto byteSwap(coda_oss::span<const T> buffer)
{
...
// Take care of treating std::complex<T> as T[]
template <typename T> inline auto byteSwap(coda_oss::span<const std::complex<T>> buffer)
{
    return byteSwap(details::make_span(buffer));
}
...
template <typename T> inline auto byteSwapValue(std::complex<T> z)
{
    // static_assert(std::is_floating_point<T>::value, "std::complex<T> should use floating-point");
...
template <typename T> inline auto byteSwap(std::complex<T> val)
{
...
struct ByteSwapRunnable final : public sys::Runnable
{
    ByteSwapRunnable(void *buffer, size_t elemSize, size_t startElement, size_t numElements) noexcept
        : mBuffer(static_cast<coda_oss::byte *>(buffer) + startElement * elemSize), mElemSize(elemSize),
...
{
    ByteSwapCopyRunnable(const void *buffer, size_t elemSize, size_t startElement, size_t numElements,
                         void *outputBuffer) noexcept
        : mBuffer(static_cast<const coda_oss::byte *>(buffer) + startElement * elemSize), mElemSize(elemSize),
          mNumElements(numElements),
