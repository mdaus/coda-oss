...
template <typename T> inline auto as_bytes(const T &v) noexcept
{
    return sys::as_bytes(&v, 1);
}
template <typename T> inline auto as_writable_bytes(T &v) noexcept
{
...
CODA_OSS_define_swapBytes_specialization_(uint8_t) // no `;`, it's not needed and generates a -Wpedantic warning
    CODA_OSS_define_swapBytes_specialization_(uint16_t) CODA_OSS_define_swapBytes_specialization_(uint32_t)
...
template <typename T> inline auto byteSwapValue(T in, coda_oss::span<coda_oss::byte> outBytes)
{
    return byteSwapValue<T>(details::as_bytes(in), outBytes);
}
template <typename T> inline auto byteSwapValue(T in)
{
...
template <typename T> inline auto byteSwapValue(coda_oss::span<const coda_oss::byte> in)
{
