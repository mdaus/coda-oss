...
std::vector<std::string> AbstractOS::search(const std::vector<std::string> &searchPaths, const std::string &fragment,
                                            const std::string &extension, bool recursive) const
{
    ...
    if (!fragment.empty() && !extension.empty())
    {
        ...
        elementsFound = sys::FileFinder::search(logicPred, searchPaths, recursive);
    }
    else if (!extension.empty())
    {
        sys::ExtensionPredicate extPred(extension);
        elementsFound = sys::FileFinder::search(extPred, searchPaths, recursive);
    }
    else if (!fragment.empty())
    {
        sys::FragmentPredicate fragPred(fragment);
        elementsFound = sys::FileFinder::search(fragPred, searchPaths, recursive);
    }
    return elementsFound;
}

std::vector<coda_oss::filesystem::path> AbstractOS::search(const std::vector<coda_oss::filesystem::path> &searchPaths,
...
void AbstractOS::remove(const std::string &path) const
{
    if (isDirectory(path))
    {
        ...
        for (DirectoryEntry::Iterator iter = dirEntry.begin(); iter != dirEntry.end(); ++iter)
        {
...
std::string AbstractOS::getCurrentExecutable(const std::string &argvPathname) const
{
    if (argvPathname.empty())
    {
    ...
    std::string candidatePathname = sys::Path::joinPaths(getCurrentWorkingDirectory(), argvPathname);
    ...
    if (splitEnv("PATH", pathDirs))
    {
        ...
        {
            candidatePathname = sys::Path::joinPaths(sys::Path::absolutePath(pathDir), argvPathname);
            if (exists(candidatePathname))
            {
                return candidatePathname;
            }
...
static bool splitEnv_(const AbstractOS &os, const std::string &envVar, std::vector<std::string> &result,
...
static const std::map<std::string, get_env_fp> s_get_env{
    ...
    {"ARGV0", nullptr},
    {"$", getSpecialEnv_PID},
    {"PID", getSpecialEnv_PID},
    {"PWD", nullptr},
    {"USER", getSpecialEnv_USER},
    {"USERNAME", getSpecialEnv_USER},
    {"HOME", getSpecialEnv_HOME},
    {"USERPROFILE", getSpecialEnv_HOME},
