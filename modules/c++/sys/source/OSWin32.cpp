...
bool sys::OSWin32::exists(const std::string &path) const
{
    ...
    if (what == INVALID_FILE_ATTRIBUTES)
    {
        ...
        {
            char *err = nullptr;
            FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, nullptr, errCode,
                          MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&err, 0, nullptr);
            throw except::Exception(
                Ctxt("Problem while testing file existence for " + path + " with Error: " + std::string(err)));
        }
        return false;
    }
...
bool sys::OSWin32::move(const std::string &path, const std::string &newPath) const
{
    ...
    return (MoveFileEx(path.c_str(), newPath.c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH)) ? (true)
                                                                                                           : (false);
}
...
static std::string getEnv(const std::string &s)
{
    ...
    if (retVal + 1 != size)
    {
        throw sys::SystemException(Ctxt("Environment variable size does not match allocated size for " + s));
    }
    if (retVal == 0)
    {
...
{
#pragma warning(push)
#pragma warning(disable : 4996) // '...': This function or variable may be unsafe. Consider using _dupenv_s instead. To
...
{
    throw except::NotImplementedException(Ctxt("Windows getNumPhysicalCPUsAvailable not yet implemented."));
}

void sys::OSWin32::getAvailableCPUs(std::vector<int> & /*physicalCPUs*/, std::vector<int> & /*htCPUs*/) const
{
...
void sys::OSWin32::createSymlink(const std::string &origPathname, const std::string &symlinkPathname) const
{
    ...
    if (!CreateSymbolicLink(const_cast<char *>(symlinkPathname.c_str()), const_cast<char *>(origPathname.c_str()),
                            true))
    {
