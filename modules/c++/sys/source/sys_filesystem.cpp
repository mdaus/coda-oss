...
{
    return fs::path(lhs) /=
           rhs; // "... returns path(lhs) /= rhs." http://en.cppreference.com/w/cpp/filesystem/path/operator_slash
}
...
fs::path fs::temp_directory_path()
{
    ...
    //
    // On POSIX systems, the path may be the one specified in the environment variables TMPDIR, TMP, TEMP, TEMPDIR, and,
    // if none of them are specified, the path "/tmp" is returned.
    //
    // On Windows systems, the path is typically the one returned by GetTempPath
#if defined(_WIN32)

    // https://msdn.microsoft.com/en-us/library/b0084kay.aspx
    // "_WIN32 Defined as 1 when the compilation target is 32 - bit ARM, 64 - bit ARM, x86, or x64.Otherwise,
    // undefined."
