...
static auto copy_to(floatv v)
{
    std::vector<float> retval(v.size());
    v.copy_to(retval.begin());
    return retval;
}
...
{
    assert(magnitudes.size() == AmplitudeTableSize);
    intv first = std::distance(magnitudes.begin(), magnitudes.begin());
    const intv last = std::distance(magnitudes.begin(), magnitudes.end());
...
static auto lower_bound(std::span<const float> magnitudes, const floatv& v)
{
    intv retval{};

    auto vcl_retval = retval.v_;
    for (int i = 0; i < v.size(); i++)
    {
        const auto it = std::lower_bound(magnitudes.begin(), magnitudes.end(), v[i]);
        const auto result = static_cast<int>(std::distance(magnitudes.begin(), it));
        vcl_retval.insert(i, result);
    }
    retval = vcl_retval;
