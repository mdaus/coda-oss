...
template <typename T> static std::vector<T> make_origValues(size_t NUM_PIXELS)
{
    ::srand(334);
...
template <typename T> inline std::span<const T> as_span(const std::vector<std::byte> &bytes)
{
...
static constexpr std::byte sixteen_bytes[]{x00, x11, x22, x33, x44, x55, x66, x77,
                                           x88, x99, xAA, xBB, xCC, xDD, xEE, xFF};

template <typename TByteSpanLike1, typename TByteSpanLike2>
static void test_assert_eq_swapped(const std::string &testName, size_t sz, const TByteSpanLike1 &pValueBytes,
                                   const TByteSpanLike2 &pResultBytes)
{
...
template <typename TUInt> static void testByteSwapValues_(const std::string &testName, const void *pBytes)
{
...
TEST_CASE(testByteSwap12)
{
    ...
    constexpr std::byte twelve_bytes[]{x00, x11, x22, x33, x44, x55, x99, xAA, xBB, xDD, xEE, xFF};
...
template <typename T> static inline void six_byteSwap(const void *in, T &out)
{
...
TEST_MAIN(TEST_CHECK(testEndianness); TEST_CHECK(testByteSwapV); TEST_CHECK(testByteSwapCxV); TEST_CHECK(testByteSwap);
          TEST_CHECK(testByteSwapValues); TEST_CHECK(testByteSwapCxValue); TEST_CHECK(testByteSwap12);
