...
template <size_t _Order, typename _T = double> class Fixed1D
{
  ...
  public:
    ...
    static constexpr inline size_t order()
    {
        return _Order;
    }
     ...
     */
    template <size_t _OtherOrder> Fixed1D<_Order> &operator*=(const Fixed1D<_OtherOrder> &p)
    {
    ...
    template <size_t _OtherOrder> Fixed1D<_Order + _OtherOrder, _T> operator*(const Fixed1D<_OtherOrder, _T> &p) const
    {
    ...
    template <size_t _OtherOrder> Fixed1D<_Order, _T> &operator+=(const Fixed1D<_OtherOrder, _T> &p)
    {
        *this = *this + p;
        return *this;
    }
    ...
    template <size_t _OtherOrder> Fixed1D<_Order, _T> &operator/=(double cv)
    {
        double recip = 1.0 / cv;
        for (size_t i = 0; i <= _Order; i++)
        {
 ...
 *
 */
template <size_t _Order, typename _T> Fixed1D<_Order, _T> operator*(double cv, const Fixed1D<_Order, _T> &p)
{
    return p * cv;
}
 ...
 */
template <size_t _Order, typename _T> std::ostream &operator<<(std::ostream &out, const Fixed1D<_Order, _T> &p)
{
    for (size_t i = 0; i <= _Order; i++)
    {
        out << p[i] << "*y^" << i << " ";
