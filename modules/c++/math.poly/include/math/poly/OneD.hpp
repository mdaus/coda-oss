...
template <typename _T> _T OneD<_T>::operator()(double at) const
{
    _T ret(0.0);
    double atPwr = 1.0;
...
template <typename _T> _T OneD<_T>::integrate(double start, double end) const
{
    double ret(0.0);
    double endAtPwr = end;
...
template <>
inline math::linear::VectorN<3, double> OneD<math::linear::VectorN<3, double>>::integrate(double start,
                                                                                          double end) const
{
...
template <typename T> OneD<T> OneD<T>::derivative() const
{
    OneD<T> ret(0);
    if (order() > 0)
    {
...
template <> inline OneD<math::linear::VectorN<3, double>> OneD<math::linear::VectorN<3, double>>::derivative() const
{
...
template <typename _T> _T OneD<_T>::velocity(double at) const
{
    _T ret{};
    double atPwr = 1.0;
    const auto sz = mCoef.size();
...
template <typename _T> _T OneD<_T>::acceleration(double at) const
{
    _T ret{};
    double atPwr = 1.0;
    const auto sz = mCoef.size();
...
template <typename _T> _T &OneD<_T>::operator[](size_t i)
{
    if (i < mCoef.size())
    {
        return (mCoef[i]);
...
template <typename _T> _T OneD<_T>::operator[](size_t i) const
{
    if (i < mCoef.size())
    {
        return mCoef[i];
...
template <typename _T> std::ostream &operator<<(std::ostream &out, const OneD<_T> &p)
{
...
template <typename _T> OneD<_T> &OneD<_T>::operator*=(double cv)
{
    const auto sz = mCoef.size();
    for (size_t i = 0; i < sz; i++)
    {
...
template <typename _T> OneD<_T> OneD<_T>::operator*(double cv) const
{
    OneD<_T> ret(*this);
    ret *= cv;
    return ret;
...
template <typename _T> OneD<_T> operator*(double cv, const OneD<_T> &p)
{
    return p * cv;
}

template <typename _T> OneD<_T> &OneD<_T>::operator*=(const OneD<_T> &p)
{
...
template <typename _T> OneD<_T> OneD<_T>::operator*(const OneD<_T> &p) const
{
    OneD<_T> ret(*this);
    ret *= p;
    return ret;
...
template <typename _T> OneD<_T> OneD<_T>::power(size_t toThe) const
{
...
template <typename _T> OneD<_T> OneD<_T>::scaleVariable(double scale) const
{
    return ::math::poly::scaleVariable<OneD<_T>>(*this, scale);
}

template <typename _T> OneD<_T> OneD<_T>::truncateTo(size_t order) const
{
...
template <typename _T> OneD<_T> OneD<_T>::truncateToNonZeros(double zeroEpsilon) const
{
...
template <>
inline OneD<math::linear::VectorN<3, double>> OneD<math::linear::VectorN<3, double>>::truncateToNonZeros(
...
template <typename _T> OneD<_T> OneD<_T>::transformInput(const OneD<_T> &gx, double zeroEpsilon) const
{
    OneD<_T> newP(order());
...
template <typename _T> void OneD<_T>::copyFrom(const OneD<_T> &p)
{
...
template <typename _T> template <typename Vector_T> bool OneD<_T>::equalImpl(const Vector_T &p) const
{
    const auto sz = size();
