...
#define CODA_OSS_sys_MM256_EXTRACTF_(ymm_, i_)                                                                         \
    _mm256_cvtss_f32(_mm256_castsi256_ps(_mm256_set1_epi32(_mm256_extract_epi32(_mm256_castps_si256(ymm_), i_))))
namespace avx
{
template <typename T> inline T &mm256_extractf_(T &ymm, int i)
{
    return CODA_OSS_sys_MM256_EXTRACTF_(ymm, i);
}
template <typename T> inline float &mm256_extractf(T &ymm, int i)
{
#if defined(__GNUC__)
    return ymm[i];
