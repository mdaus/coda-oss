...
sys::SSize_T InputStream::streamTo(OutputStream &soi, sys::SSize_T bytesToPipe)
{
    ...
    while (((bytesRead = read(vec, sizeOfVec)) != io::InputStream::IS_EOF) && (totalBytesTransferred != bytesToPipe))
    {

        soi.write(vec, bytesRead);
        totalBytesTransferred += bytesRead;
        memset(vec, 0, DEFAULT_CHUNK_SIZE);
        sizeOfVec = (bytesToPipe - totalBytesTransferred <= defaultChunkSize) ? (bytesToPipe - totalBytesTransferred)
                                                                              : defaultChunkSize;
    }
