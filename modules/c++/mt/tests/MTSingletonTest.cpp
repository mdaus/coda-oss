...
public:
    ...
    {
        coutMutex.lock();
        std::cout << ("Beware the ides of March.") << std::endl;
        coutMutex.unlock();
    }
};
...
public:
    ...
    {
        coutMutex.lock();
        std::cout << ("Cowards die many times before their deaths;   The valiant never taste of death but once. 35  Of all the wonders that I yet have heard.   It seems to me most strange that men should fear;   Seeing that death, a necessary end,   Will come when it will come.") << std::endl;
        coutMutex.unlock();
    }
...
public:
    ...
    {
        coutMutex.lock();
        std::cout << ("Villains, you did not so, when your vile daggers   Hack'd one another in the sides of Caesar:   You show'd your teeth like apes, and fawn'd like hounds, 45  And bow'd like bondmen, kissing Caesar's feet;   Whilst damned Casca, like a cur, behind   Struck Caesar on the neck. O you flatterers!") << std::endl;
        coutMutex.unlock();
    }
};
...
public:
    ...
    {
        coutMutex.lock();
        std::cout << ("According to his virtue let us use him,   With all respect and rites of burial.   Within my tent his bones to-night shall lie, 85  Most like a soldier, order'd honourably.   So call the field to rest; and let's away,   To part the glories of this happy day.") << std::endl;
        coutMutex.unlock();
    }
};
