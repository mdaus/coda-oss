...
template <typename OpT> void run1D(size_t numElements, size_t numThreads, const OpT &op)
{
    ...
    else
    {
        ...
        {
            threads.createThread(new Runnable1D<OpT>(startElement, numElementsThisThread, op));
        }
        threads.joinAll();
    }
}
...
template <typename OpT> void run1D(size_t numElements, size_t numThreads, const std::vector<OpT> &ops)
{
    ...
    else
    {
        ...
        while (planner.getThreadInfo(threadNum, startElement, numElementsThisThread))
        {
            threads.createThread(new Runnable1D<OpT>(startElement, numElementsThisThread, ops[threadNum++]));
        }
        threads.joinAll();
    }
}
