...
template <typename T> void limit(T &num)
{
    while (100 < num)
    {
        num /= 10;
 ...
 */
std::complex<float> getMeanWComplex(sys::RealTimeStopWatch &wtch, const std::vector<std::complex<float>> &in,
                                    size_t sze, double &duration, size_t numLoops = 1)
{
 ...
 *      return value: The mean of the vector
 */
std::complex<float> getMeanWDouble(sys::RealTimeStopWatch &wtch, const std::vector<std::complex<float>> &in, size_t sze,
                                   double &duration, size_t numLoops = 1)
{
...
void print(std::ostream &out, size_t sze, std::complex<float> meanOne, std::complex<float> meanTwo, double durOne,
           double durTwo)
{
    out << std::setw(15) << sze << std::setw(25) << meanOne << std::setw(25) << meanTwo << std::setw(15)
        << durOne / 1000 << std::setw(25) << durTwo / 1000 << '\n';
}
...
void loopingBenchmark(size_t size, size_t growthFactor, size_t numGrowths, std::ostringstream &out)
{
    ...
    for (size_t i = 0; i < numGrowths; ++i)
    {
        ...
        for (size_t k = 0; k < NUM_TRIALS; ++k)
        {
            ...
            std::complex<float> cmplxMean = getMeanWComplex(cmplxWatch, arr, size, cmplxTime, numLoops);
...
size_t decideSize(size_t initSize, size_t growthFactor, size_t numGrowths)
{
    ...
    auto largestPosGrowth =
        static_cast<size_t>(initSize * std::pow(static_cast<double>(growthFactor), static_cast<double>(numGrowths)));
 ...
 *      out: an ostringstream that stores the output from the simulations
 */
void singlePassBenchmark(size_t size, size_t growthFactor, size_t numGrowths, std::ostringstream &out)
{
