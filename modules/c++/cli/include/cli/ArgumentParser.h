...
class CODA_OSS_API ArgumentParser
{
  public:
     ...
     */
    std::shared_ptr<Argument> addArgument(const std::string &nameOrFlags, const std::string &help = "",
                                          cli::Action action = cli::STORE, const std::string &destination = "",
                                          const std::string &metavar = "", int minArgs = -1, int maxArgs = -1,
                                          bool required = false);
     ...
     */
    void printUsage(std::ostream &out = std::cerr, bool andExit = false, const std::string &message = "") const;
