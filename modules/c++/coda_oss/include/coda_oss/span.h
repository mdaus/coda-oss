...
template <typename T> inline auto as_bytes(span<const T> s) noexcept
{
...
template <typename T> inline auto as_bytes(span<T> s) noexcept
{
    const span<const T> s_(s.data(), s.size());
    return as_bytes(s_);
}
template <typename T> inline span<byte> as_writable_bytes(span<T> s) noexcept
{
    // https://en.cppreference.com/w/cpp/types/is_trivially_copyable "... serialized to/from binary files ..."
    static_assert(std::is_trivially_copyable<T>::value, "must be 'trivially' copyable.");
...
template <typename C> constexpr size_t size(const C &c)
{
    return c.size();
}
template <typename C> constexpr ptrdiff_t ssize(const C &c)
{
...
template <typename T, ptrdiff_t N> constexpr ptrdiff_t ssize(const T (&)[N]) noexcept
{
    return N;
}
